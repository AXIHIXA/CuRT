...
)
{
    ...
    list[2] = new Sphere({1.0f, 0.0f, -1.0f}, 0.5f, new Metal({0.8f, 0.6f, 0.2f}, 1.0f));
    list[3] = new Sphere({-1.0f, 0.0f, -1.0f}, 0.5f, new Metal({0.8f, 0.8f, 0.8f}, 0.3f));
    *world = new World(list, 4);
    *camera = new Camera();
}
...
int main(int argc, char * argv[])
{
    ...
    thrust::device_vector<void *> dPtrPool(6);
    Hitable ** dList = reinterpret_cast<Hitable **>(dPtrPool.data().get());
    Hitable ** dWorld = dList + 4;
    Camera ** dCamera = reinterpret_cast<Camera **>((dPtrPool.data() + 5).get());
    createWorld<<<1U, 1U>>>(
            dList,
            dWorld,
            dCamera
